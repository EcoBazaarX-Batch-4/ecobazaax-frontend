...
const CreateProduct = () => {
  ...
  const [staticData, setStaticData] = useState({ categories: [], materials: [], manufacturing: [], packaging: [] });
  ...
  useEffect(() => {
    const loadConfig = async () => {
      try {
        const [cat, mat, mfg, pkg] = await Promise.all([
          adminService.getCategories(), adminService.getMaterials(),
          adminService.getManufacturingProcesses(), adminService.getPackagingMaterials()
        ]);
        setStaticData({ categories: cat.data || [], materials: mat.data || [], manufacturing: mfg.data || [], packaging: pkg.data || [] });
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      if (formData.materials.length === 0) throw new Error("Add at least one material.");
      ...
      <Paper sx={{ p: 4 }}>
        <form onSubmit={handleSubmit}>
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={4}><TextField fullWidth type="number" label="Price" name="price" value={formData.price} onChange={handleChange} required /></Grid>
            <Grid item xs={12} md={4}><TextField fullWidth type="number" label="Stock" name="stockQuantity" value={formData.stockQuantity} onChange={handleChange} required /></Grid>
          ...
          <Box mt={4} display="flex" justifyContent="flex-end" gap={2}>
            <Button type="submit" variant="contained" disabled={loading} size="large">{loading ? "Creating..." : "Create Product"}</Button>
          </Box>
        </form>
      </Paper>
    </Container>
