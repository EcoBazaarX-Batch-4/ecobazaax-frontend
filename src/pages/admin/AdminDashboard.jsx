    ...
    <Box>
      ...
      <Grid container spacing={3} mb={4}>
        <Grid item xs={12} sm={6} md={3}>
          {/* FIX: Map 'totalCustomers' to Users card */}
          <StatCard title="Total Users" value={insights?.totalCustomers || 0} icon={<People sx={{ fontSize: 32 }} />} color="primary" />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
        ...
        <Grid item xs={12} sm={6} md={3}>
          {/* FIX: Use 'totalPlatformRevenue' */}
          <StatCard title="Revenue" value={`₹${insights?.totalPlatformRevenue?.toFixed(2) || 0}`} icon={<TrendingUp sx={{ fontSize: 32 }} />} color="success" />
        </Grid>
      </Grid>
