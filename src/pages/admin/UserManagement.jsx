...
import {
  Box, Typography, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Chip, IconButton, Skeleton, Alert
} from "@mui/material";
...
const UserManagement = () => {
  ...
  const loadUsers = async () => {
    try {
      ...
      // FIX: Spring Boot Page object puts list in 'content'
            ...
            {loading ? [...Array(5)].map((_, i) => <TableRow key={i}><TableCell colSpan={6}><Skeleton /></TableCell></TableRow>) :
             users.map((user) => (
              <TableRow key={user.id} hover>
                ...
                <TableCell>
                   {/* FIX: Handle role array display */}
                   {user.roles?.map(r => <Chip key={r} label={r.replace('ROLE_', '')} size="small" sx={{ mr: 0.5 }} />)}
                </TableCell>
                <TableCell align="center">
                  <Chip label={user.isAccountNonLocked ? "Active" : "Locked"} color={user.isAccountNonLocked ? "success" : "error"} size="small" />
                </TableCell>
                <TableCell align="center">
